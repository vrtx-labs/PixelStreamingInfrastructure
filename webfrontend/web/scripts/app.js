...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  webRtcPlayerObj.onVideoInitialised = function () {
    if (ws && ws.readyState === WS_OPEN_STATE) {
      if (shouldShowPlayOverlay) {
        try {
          // certain times we cannot directly start because https://developer.chrome.com/blog/autoplay/ 
          hookStartStream();
        } catch (e) {
          showPlayOverlay();
        }
  ...
  webRtcPlayerObj.onDataChannelConnected = function () {
    ...
    /**
     * add observer for closing the data channel
...
function onWebRtcAnswer(webRTCData) {
  ...
  webRtcPlayerObj.onAggregatedStats = (aggregatedStats) => {
    ...
    if (VideoEncoderQP > redQP) {
      ...
      statsText += `<div class="${color}Status">Bad network connection</div>`;
    ...
    statsText += `<div class="${color}Status">Bitrate (kbps): ${aggregatedStats.hasOwnProperty('bitrate') ? numberFormat.format(aggregatedStats.bitrate) : 'Chrome only'}</div>`;
...
function connect() {
  ...
  ws.onerror = function (event) {
    ...
    dispatchStateNotice(false, { type: 'ws', status: 'error' });
  };
  ...
  ws.onclose = function (event) {
    ...
    dispatchStateNotice(false, { type: 'ws', status: 'closed' });
...
/**
 * emit state change information to listeners (parent)
 * @param {object} detail 
 * @author Matt Vander Vliet
 */
...
/**
 * send a request through the websocket connection into the gaming system APIs
 * @param {string} action name of the API call
 * @param {object} [payload] optional payload to send
 * @see signalserver/wrapper.js:166
 * @custom
 */
function emitSocketInteraction(action, payload) {
  if (ws && ws.readyState === WS_OPEN_STATE) {
    ws.send(JSON.stringify({
      type: 'control', // signalserver/wrapper.js:166
      action: action,
      payload: payload
    }));
  }
...
/**
 * send socket based responses to any subscribers
 ...
 *         Customizations IIFE
 * -------------------------------------------
 * - 2021.09.30 MV: add hooks into web apps
...
(function(window, document, custom) {
  ...
  // add communicator to window.parent
  if (top !== window) {
  ...
  // add communicator from window.parent
  window.addEventListener('message', function (event) {
    if (!/(localhost|https)/.test(event.origin)) {
      return;
    }
    ...
    if (msg.invoke) {
      ...
      if (fn) { 
        ...
        console.warn('Receieved msg to invoke missing function', msg.invoke);
      }
    } else {
      console.warn('Unhandled message in player', msg);
    }
  });
